...
export default function TrackingPage() {
  const userIssues = issues.filter(issue => issue.reporter === "You");
                        ...
                        <div className="flex items-center justify-between w-full">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    ...
                                    ID: {issue.id} • Reported on {new Date(issue.reportedAt).toLocaleDateString()}
                                    </p>
                                </div>
                            </div>
                                    ...
                                    {issue.updates.map((update, index) => (
                                        <li key={index} className="mb-6 ml-6">            
                                            ...
                                            <div className="p-4 bg-muted/50 rounded-lg border">
                                                ...
                                                <time className="block mb-2 text-xs font-normal leading-none text-muted-foreground">{new Date(update.date).toLocaleString()}</time>
                            ...
                            {issue.status === 'Resolved' && issue.resolutionUrl && (
                                <div>
                                    ...
                                    <div className="relative aspect-video w-full max-w-md overflow-hidden rounded-lg">
                                        <Image src={issue.resolutionUrl} data-ai-hint={issue.resolutionHint} alt={`Resolution for ${issue.title}`} layout="fill" objectFit="cover" />
                                    </div>
                                </div>
                            )}
                        </div>
