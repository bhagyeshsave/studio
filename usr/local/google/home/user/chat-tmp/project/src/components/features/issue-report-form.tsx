...
export function IssueReportForm() {
  ...
  async function onSubmit(values: z.infer<typeof issueSchema>) {
    ...
    // Destructure media out, so we only pass text data to our addIssue function
    const { media, ...issueData } = values;

    try {
      // Call addIssue with only the text data. The media file is ignored.
      await addIssue(issueData);
      toast({
        title: "Issue Reported!",
        description: "Thank you for helping improve your community. Media upload is temporarily disabled.",
      });
      form.reset();
      setPreview(null);
      setFileType(null);
  ...
  return (
    ...
    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        ...
        <FormField
            ...
            render={({ field }) => (
                <FormItem>
                <FormLabel>Photo/Video (Temporarily Disabled)</FormLabel>
                <FormControl>
                ...
                <FormDescription>
                    Adding a photo or video can help resolve the issue faster. This is temporarily disabled for debugging.
                </FormDescription>
                <FormMessage />
                </FormItem>
            )}
